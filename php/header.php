...
<?php
	$search='';
	$displaytop='<li><a href="login.php">login</a></li>';
	if (isset($_SESSION['id'])) {
		...
		if (isset($_SESSION['l_name']) and isset($_SESSION['job_position'])) {
			...
			if ($job_position=="assistant" or $job_position=="admin") {
				$displaytop='<li><a href="staff_profile.php">'.$last_name.'</a></li>';
				$displaytop.='<li><a href="staff_interface.php">work place</a></li>';
				$query="SELECT member_id,book_id,extend_type FROM extend_request";
				$rquery=mysqli_query($admin_conn,$query);
				if ($rquery) {
					if (mysqli_num_rows($rquery)>0) {
						$ncount=mysqli_num_rows($rquery);
						$displaytop.='<li><a href="notification.php"><div class="blink">'.$ncount.' notification(s)</div></a></li>';
					}
				}
			}
			if ($job_position=="member") {
				$displaytop='<li><a href="member_profile.php">'.$last_name.'</a></li>';
			}
		}
		$displaytop.='<li><a href="cart.php">cart</a></li>';
		$displaytop.='<li><a href="logout.php">logout</a></li>';
	}
